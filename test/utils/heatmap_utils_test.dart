...
void main() {
  group('HeatmapUtils.dayKey', () {
    ...
    group('Timezone Scenarios', () {
      ...
      test('handles edge-of-day times correctly', () {
        ...
        expect(
          HeatmapUtils.dayKey(justAfterMidnight),
        ...
        expect(
          HeatmapUtils.dayKey(justBeforeMidnight),
          equals(DateTime.utc(2025, 9, 25)),
        );
      });
    ...
    group('DST Transition Scenarios', () {
      test('handles spring forward dates (DST begins)', () {
        ...
        expect(
          HeatmapUtils.dayKey(beforeDST),
          equals(DateTime.utc(2025, 3, 8)),
        );
        expect(
          HeatmapUtils.dayKey(duringDST),
          equals(DateTime.utc(2025, 3, 9)),
        );
        expect(
          HeatmapUtils.dayKey(afterDST),
          equals(DateTime.utc(2025, 3, 10)),
        );
      });
      ...
      test('handles fall back dates (DST ends)', () {
        ...
        final duringFallBack = DateTime(
          2025,
          11,
          2,
          1,
          30,
        ); // During repeated hour
        final afterFallBack = DateTime(2025, 11, 3, 14, 30);

        expect(
          HeatmapUtils.dayKey(beforeFallBack),
          equals(DateTime.utc(2025, 11, 1)),
        );
        expect(
          HeatmapUtils.dayKey(duringFallBack),
          equals(DateTime.utc(2025, 11, 2)),
        );
        expect(
          HeatmapUtils.dayKey(afterFallBack),
          equals(DateTime.utc(2025, 11, 3)),
        );
      });
    ...
    group('Edge Cases and Boundary Conditions', () {
      test('handles leap year dates', () {
        ...
        expect(
          HeatmapUtils.dayKey(beforeLeap),
          equals(DateTime.utc(2024, 2, 28)),
        );

        // Day after leap day
        final afterLeap = DateTime(2024, 3, 1, 12, 0);
        expect(
          HeatmapUtils.dayKey(afterLeap),
          equals(DateTime.utc(2024, 3, 1)),
        );
      });
      ...
      test('handles year boundaries', () {
        ...
        expect(
          HeatmapUtils.dayKey(newYearsEve),
          equals(DateTime.utc(2024, 12, 31)),
        );
        expect(
          HeatmapUtils.dayKey(newYearsDay),
      ...
      test('handles month boundaries', () {
        ...
        expect(
          HeatmapUtils.dayKey(endOfMonth),
          equals(DateTime.utc(2025, 1, 31)),
        );
        expect(
          HeatmapUtils.dayKey(startOfNextMonth),
          equals(DateTime.utc(2025, 2, 1)),
        );
      });
      ...
      test('handles extreme dates', () {
        ...
        expect(
          HeatmapUtils.dayKey(earlyDate),
          equals(DateTime.utc(1900, 1, 1)),
        );

        // Far future date
        final futureDate = DateTime(2100, 12, 31, 12, 0);
        expect(
          HeatmapUtils.dayKey(futureDate),
          equals(DateTime.utc(2100, 12, 31)),
        );
      });
    });
  ...
  group('HeatmapUtils.alignToWeekStart', () {
    test('aligns dates to Monday start correctly', () {
      ...
      final mondayStart = HeatmapUtils.alignToWeekStart(
        wednesday,
        DateTime.monday,
      );
    ...
    test('aligns dates to Sunday start correctly', () {
      // Wednesday, January 17, 2024
      final wednesday = DateTime(2024, 1, 17);
      final sundayStart = HeatmapUtils.alignToWeekStart(
        wednesday,
        DateTime.sunday,
      );
  ...
  group('HeatmapUtils.isFirstWeekdayOfMonth', () {
    test('identifies first week of month correctly', () {
      ...
      expect(
        HeatmapUtils.isFirstWeekdayOfMonth(DateTime(2024, 1, 15)),
        isFalse,
      );
    });
  });
  ...
  group('HeatmapUtils.defaultColorScale', () {
    test('returns correct colors for different contribution levels', () {
      expect(
        HeatmapUtils.defaultColorScale(0),
        equals(const Color(0xFFFFE4BC)),
      );
      expect(
        HeatmapUtils.defaultColorScale(-1),
        equals(const Color(0xFFFFE4BC)),
      );
      ...
      expect(
        HeatmapUtils.defaultColorScale(10),
        equals(Colors.orange.shade800),
      );
      expect(
        HeatmapUtils.defaultColorScale(15),
